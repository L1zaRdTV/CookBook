...
namespace CookBook
{
    ...
    public class Recipes { [Key] public int RecipeID { get; set; } public string RecipeName { get; set; } public string Description { get; set; } public int? CategoryID { get; set; } public int? AuthorID { get; set; } public int? CookingTime { get; set; } public string Image { get; set; } [NotMapped] public string CategoryDisplay { get; set; } [NotMapped] public string CurrentPhoto { get { if (!string.IsNullOrEmpty(Image)) { string path = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\Images\", Image); if (System.IO.File.Exists(path)) return path; } return "/Resources/default_recipe.png"; } } }
